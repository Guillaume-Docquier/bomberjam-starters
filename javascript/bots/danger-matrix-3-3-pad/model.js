...
*/
const modelName = "danger-matrix-3-3-pad";
...
function buildModel() {
    ...
    // Convolutions
    // S1
    model.add(tf.layers.conv2d({ inputShape: DATA_SHAPE, dataFormat: "channelsFirst", filters: 32, kernelSize: 3, activation: "relu", padding: "same" }));
    model.add(tf.layers.dropout({ rate : 0.15 }));

    model.add(tf.layers.conv2d({ dataFormat: "channelsFirst", filters: 32, kernelSize: 3, activation: "relu", padding: "same" }));
    model.add(tf.layers.dropout({ rate : 0.15 }));
